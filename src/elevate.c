...
BOOL tryElevate(HWND hWnd, BOOL silent) {
    ...
    if (!IsWindowsVistaOrGreater()) {
        if (!silent) MessageBox(hWnd, (LPCSTR)APP_NAME " only supports Windows Vista or above.",
        ...
        if (GetModuleFileName(NULL, (LPSTR)szPath, ARRAYSIZE(szPath)))
        {
            ...
            if (!ShellExecuteEx(&sei))
            {
                ...
                if (dwError == ERROR_CANCELLED)
                {
                    ...
                    // alert and exit
                    MessageBox(hWnd, (LPCSTR)APP_NAME " needs to be elevated to work. Run as Administrator or click Yes in the UAC dialog.",
                        (LPCSTR)"Aborting", MB_OK);
                }
            }
            // runas executed.
        } else {
            MessageBox(hWnd, (LPCSTR)"Failed to get " APP_NAME " path. Please place the executable in a normal directory.",
                (LPCSTR)"Aborting", MB_OK);
        }
    }
